...
class Costumer:
    ...
    def Add_product(self, new_product: Product):
        if new_product.name_product in self.shopping_list:
            self.shopping_list[new_product.name_product][
                "unit"
            ] += new_product.amount_unit
            self.receipt += (
                self.shopping_list[new_product.name_product]["price"]
                * new_product.amount_unit
            )
        else:
            self.shopping_list[new_product.name_product] = {
                "product": new_product.name_product,
                "price": new_product.price_for_unit,
                "unit": new_product.amount_unit,
            }
    ...
    def Remove_product(self, new_product: Product):
        if new_product.name_product in self.shopping_list:
            if (
                new_product.amount_unit
                < self.shopping_list[new_product.name_product]["unit"]
            ):
                self.shopping_list[new_product.name_product][
                    "unit"
                ] -= new_product.amount_unit
